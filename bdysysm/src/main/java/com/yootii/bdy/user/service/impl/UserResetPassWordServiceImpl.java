...
public class UserResetPassWordServiceImpl implements UserResetPassWordService {
	 ...
	 */
	private String createLink(HttpServletRequest request, User user,GeneralCondition gcon)
			throws Exception {
		...
		user.setValidDate(outDate);
		user.setValidataCode(secretKey);
		userServiceImpl.modifyUserSelf(user,null);
		...
		if (request != null) {
			...
			if (restP != null && !restP.equals("")) {
				...
				resetPassPath = path+ "/interface/user/tofindpassword";
			}
		}
	...
	public ReturnInfo validUser(HttpServletRequest request,User userParam,
		...
		try {
			...
			User user = authenticationService.authentication(userParam,
					gcon, false);		
			if (user == null) {
				rtnInfo.setSuccess(false);
			...
			// 生成带有重置密码链接的邮件
			String mailContent = createLink(request, user,gcon);
	...
	private ReturnInfo findPasswordInternal(HttpServletRequest request) throws Exception {
		...
		if (user != null) {
			...
			// 获取当前登陆人的加密码
			String vailddataConde = user.getValidataCode();
